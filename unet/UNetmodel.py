...
class DownConv(nn.Module):
    """
    Encoder block has 2 convolution layers and 1 maxbilinear layer.
    ...
    padding in maxbilinear layer so that the size of feature maps is matching
    in decoder.
    ...
    def __init__(self, n_channels, out_channels, bilinear=True):
        ...
        self.conv1 = nn.Conv2d(n_channels, out_channels, kernel_size=3, padding=1)
...
class UpConv(nn.Module):
    ...
    def __init__(self, n_channels, out_channels):
        ...
        self.upconv = nn.ConvTranspose2d(n_channels, out_channels, kernel_size=2, stride=2)
...
class UNet(nn.Module):
    ...
    def __init__(self, n_classes, n_channels=3, depth=5, start_filts=64):
        """
        Arguments:
            n_channels: int, number of channels in the input tensor. If input
        ...
        for i in range(depth):
            if i == 0:
                temp_n_channels = self.n_channels
            else:
                 temp_n_channels = temp_out_channels
            temp_out_channels = self.start_filts*(2**i)
            bilinear = True if i < depth-1 else False
            encoder_block = DownConv(temp_n_channels, temp_out_channels, bilinear=bilinear)
        ...
        for k in range(depth-1):
            temp_n_channels = temp_out_channels
            temp_out_channels = temp_n_channels // 2
            decoder_block = UpConv(temp_n_channels, temp_out_channels)
            self.up_convs.append(decoder_block)

        self.conv_final = nn.Conv2d(temp_out_channels, self.n_classes, kernel_size=1)
